...
Aesthetic: mythological dark — black void, gold/parchment, stone texture, Cinzel typography.
"""
...
<header class="site-header">
  ...
  <div class="header-meta">
    ...
    <span style="color:var(--gold-dim)">%%CHRONICLE_COUNT%% runs en chronicle</span>
  </div>
</header>

<!-- I Hardware -->
<div class="rune-header"><span class="rune-num">I</span><span class="rune-title">Equipamiento</span><div class="rune-line"></div></div>
<div class="hw-grid">
  ...
  <div class="hw-cell"><div class="hw-label">Procesador</div><div class="hw-val" style="font-size:.82rem">%%CPU_NAME%%</div></div>
  <div class="hw-cell"><div class="hw-label">Núcleos CPU</div><div class="hw-val">%%CPU_CORES%%</div></div>
  <div class="hw-cell"><div class="hw-label">RAM Sistema</div><div class="hw-val">%%RAM_GB%% GB</div></div>
  <div class="hw-cell"><div class="hw-label">Servidor LLM</div><div class="hw-val">%%SERVER_NAME%%</div></div>
  <div class="hw-cell"><div class="hw-label">Modelo</div><div class="hw-val" style="font-size:.78rem">%%MODEL%%</div></div>
  <div class="hw-cell"><div class="hw-label">Modo benchmark</div><div class="hw-val">%%MODE%%</div></div>
</div>

<!-- II Métricas -->
<div class="rune-header"><span class="rune-num">II</span><span class="rune-title">Métricas del combate</span><div class="rune-line"></div></div>
<div class="stat-row">
  <div class="stat-card">
    <div class="stat-icon">⚡</div>
    <div class="stat-label">TPS — Velocidad base</div>
    <div class="stat-val" id="kpi-tps">—</div>
    <div class="stat-unit">tokens por segundo</div>
    <div class="stat-lore"><strong>¿Qué es TPS?</strong> Cuántas palabras genera el modelo por segundo. 10 t/s se siente fluido. Menos de 3 t/s es lento. Este valor es tu techo <em>sin presión</em> de contexto.</div>
  </div>
  <div class="stat-card">
    <div class="stat-icon">🧠</div>
    <div class="stat-label">VRAM en el pico</div>
    <div class="stat-val" id="kpi-vram" style="color:%%VRAM_COLOR%%">%%PEAK_VRAM_MB%%</div>
    <div class="stat-unit">MB de %%VRAM_TOTAL_MB%% disponibles</div>
    <div class="meter">
      <div class="meter-header"><span>uso VRAM</span><span>%%VRAM_PCT%%%</span></div>
      <div class="meter-track"><div class="meter-fill" id="vram-bar" style="width:0%;background:%%VRAM_COLOR%%"></div></div>
    </div>
    <div class="stat-lore"><strong>VRAM</strong> es donde vive el modelo. Si se llena, el sistema crashea (OOM) o cae a RAM del sistema, que es ~10× más lenta.</div>
  </div>
  <div class="stat-card">
    <div class="stat-icon">🔥</div>
    <div class="stat-label">Temperatura pico GPU</div>
    <div class="stat-val" style="color:%%TEMP_COLOR%%">%%PEAK_TEMP%%</div>
    <div class="stat-unit">temperatura máxima registrada</div>
    <div class="stat-lore">Por encima de 83°C la GPU hace <em>thermal throttling</em> y baja sus frecuencias automáticamente. Revisá el cooling o bajá el power limit si llegás ahí.</div>
  </div>
  <div class="stat-card">
    <div class="stat-icon">⚙️</div>
    <div class="stat-label">Utilización GPU</div>
    <div class="stat-val">%%AVG_UTIL%%</div>
    <div class="stat-unit">promedio durante el benchmark</div>
    <div class="stat-lore">100% = la GPU trabaja a fondo (ideal). Si ves valores bajos, el cuello de botella está en <strong>CPU o en el servidor LLM</strong>, no en la GPU.%%POWER_STR%%</div>
  </div>
</div>

<!-- III Gráficos -->
<div class="rune-header"><span class="rune-num">III</span><span class="rune-title">Análisis de rendimiento</span><div class="rune-line"></div></div>
<div class="tab-strip">
  <button class="tab-btn active" onclick="switchTab('tps',this)">TPS vs Contexto</button>
  <button class="tab-btn" onclick="switchTab('lat',this)">Latencia</button>
  <button class="tab-btn" onclick="switchTab('hist',this)">Histórico</button>
</div>
<div id="tab-tps" class="tab-pane active">
  <div class="panel">
    <div class="panel-title">Tokens/s vs Tamaño de Contexto</div>
    <p class="panel-lore">El TPS <strong>decae a medida que crece el contexto</strong>. El transformer calcula atención sobre todos los tokens previos — más contexto, más cómputo. La caída pronunciada señala el inicio del <strong>memory wall</strong>.</p>
    <div class="chart-wrap"><canvas id="chart-tps"></canvas></div>
  </div>
</div>
<div id="tab-lat" class="tab-pane">
  <div class="panel">
    <div class="panel-title">Latencia vs Tamaño de Contexto</div>
    <p class="panel-lore">Tiempo total de respuesta en segundos. Si la latencia se dispara <strong>desproporcionalmente</strong> en cierto punto, puede ser señal de memory swapping entre VRAM y RAM del sistema.</p>
    <div class="chart-wrap"><canvas id="chart-lat"></canvas></div>
  </div>
</div>
<div id="tab-hist" class="tab-pane">
  <div class="panel">
    <div class="panel-title">Histórico acumulado — Chronicle</div>
    <p class="panel-lore">Todos los benchmarks guardados localmente. Cada punto es una ejecución pasada. Útil para ver <strong>consistencia entre runs</strong> y detectar regresiones de performance.</p>
    <div class="chart-wrap"><canvas id="chart-hist"></canvas></div>
  </div>
</div>

<!-- IV Detalle -->
<div class="rune-header"><span class="rune-num">IV</span><span class="rune-title">Detalle de pruebas</span><div class="rune-line"></div></div>
<div class="panel" style="margin-bottom:0">
  <p class="panel-lore" style="margin-bottom:18px"><strong>Baseline</strong> establece el TPS máximo. <strong>Context sweep</strong> mapea la caída de performance. <strong>Stress</strong> detecta caída sostenida en generaciones largas. <strong>Memory wall</strong> busca tu límite real de VRAM.</p>
  <div class="scroll-x">
    <table class="bench-table">
      <thead><tr><th>Prueba</th><th>Categoría</th><th>Ctx</th><th>TPS prom.</th><th>TPS mejor</th><th>Tiempo</th><th>Runs OK</th></tr></thead>
...
<!-- V Conocimiento -->
<div class="rune-header"><span class="rune-num">V</span><span class="rune-title">Grimorios del setup local</span><div class="rune-line"></div></div>
<div class="scrolls-grid">
  <div class="scroll"><div class="scroll-title">Context Window</div><div class="scroll-text">Tokens que el modelo puede "ver" a la vez. Más contexto = conversaciones más largas, pero <strong>consume exponencialmente más VRAM</strong> y baja el TPS. Con 24GB podés correr contextos de 32K–64K según el modelo y cuantización.</div></div>
  <div class="scroll"><div class="scroll-title">Flash Attention</div><div class="scroll-text">Optimización del cálculo de atención que reduce VRAM y mejora TPS en un <strong>20–50%</strong> en contextos grandes. Activala siempre: <code>--flash-attn on</code> en llama-server.</div></div>
  <div class="scroll"><div class="scroll-title">Cuantización</div><div class="scroll-text">Reduce la precisión de los pesos (FP16 → Q8 → Q6 → Q4). <strong>Q4_K_M</strong>: sweet spot habitual. ~20% pérdida de calidad vs ~60% menos VRAM respecto a FP16.</div></div>
  <div class="scroll"><div class="scroll-title">Memory Wall</div><div class="scroll-text">Cuando el modelo no entra en VRAM y spilova a RAM del sistema. El TPS puede <strong>caer 10×</strong>. Typhon detecta este punto automáticamente para que conozcas tu límite real.</div></div>
  <div class="scroll"><div class="scroll-title">KV Cache</div><div class="scroll-text">La caché de keys y values durante la generación. Crece linealmente con el contexto. Es el <strong>principal consumidor de VRAM</strong> en contextos largos. Reducir <code>--ctx-size</code> es la solución directa.</div></div>
  <div class="scroll"><div class="scroll-title">Thermal Throttling</div><div class="scroll-text">Por encima de ~83°C la GPU baja sus frecuencias para protegerse. TPS bajan sin razón aparente. Solución: mejor airflow, pasta térmica, o <code>nvidia-smi -pl &lt;watts&gt;</code> para bajar el power limit.</div></div>
</div>

<!-- VI Presagios -->
<div class="rune-header"><span class="rune-num">VI</span><span class="rune-title">Presagios del Oráculo</span><div class="rune-line"></div></div>
<div id="omens-container"></div>

<footer class="site-footer"><p>TYPHON v2.0 &nbsp;·&nbsp; github.com/IAcriolla/typhon-stress-test &nbsp;·&nbsp; MIT License</p></footer>
</div>
...
new Chart(document.getElementById('chart-tps'),{
  ...
  data:{
    labels:CTX_FMT,
    datasets:[{label:'TPS',data:TPS_VALUES,
      borderColor:'#c8a84b',backgroundColor:'rgba(200,168,75,0.05)',
      borderWidth:2,tension:0.35,fill:true,
      pointBackgroundColor:'#c8a84b',pointBorderColor:'#09090f',pointBorderWidth:2,
      pointRadius:5,pointHoverRadius:7}]
  },
  options:{responsive:true,maintainAspectRatio:false,
    plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>` ${c.raw.toFixed(1)} t/s`}}},
    scales:{
      x:{grid:GRID,title:{display:true,text:'Contexto (tokens)',color:'#52506a'}},
...
new Chart(document.getElementById('chart-lat'),{
  type:'bar',
  data:{labels:CTX_FMT,datasets:[{label:'Latencia (s)',data:ELAPSED,
    backgroundColor:'rgba(184,48,32,0.4)',borderColor:'#b83020',borderWidth:1,borderRadius:3}]},
  options:{responsive:true,maintainAspectRatio:false,
...
if(HIST_ROWS.length > 0){
  new Chart(document.getElementById('chart-hist'),{
    type:'scatter',
    data:{datasets:[{label:'TPS',
      data:HIST_ROWS.map(r=>({x:r.ctx_size,y:r.avg_tps})),
      backgroundColor:'rgba(200,168,75,0.5)',pointRadius:4,pointHoverRadius:6}]},
    options:{responsive:true,maintainAspectRatio:false,
      plugins:{legend:{display:false}},
      scales:{x:{grid:GRID,title:{display:true,text:'ctx tokens',color:'#52506a'}},
              y:{grid:GRID,beginAtZero:true,title:{display:true,text:'t/s',color:'#52506a'}}}}
  });
} else {
  document.getElementById('chart-hist').closest('.chart-wrap').innerHTML =
    `<p style="color:#323048;text-align:center;padding:80px 0;font-family:'Share Tech Mono',monospace;font-size:.76rem">NO HAY DATOS HISTÓRICOS AÚN<br><br><span style="color:#252535">Corré más benchmarks para ver la evolución.</span></p>`;
}
...
(function(){
  const os=[];
  if(VRAM_PCT>90) os.push({t:'danger',g:'⛔',ti:'VRAM al límite — peligro de OOM',tx:`Usás más del 90% de VRAM. Alto riesgo de crash. Reducí <code>--ctx-size</code> o cambiá a cuantización más agresiva (<strong>Q4_K_M</strong>).`});
  else if(VRAM_PCT>75) os.push({t:'warn',g:'⚠️',ti:'VRAM elevada — margen reducido',tx:`Usás el ${VRAM_PCT}% de VRAM. Activá <code>--flash-attn on</code> si no lo hiciste — puede reducir el uso de VRAM un 15–30%.`});
  if(BASELINE<5) os.push({t:'warn',g:'🐢',ti:'TPS bajo — experiencia lenta',tx:`${BASELINE.toFixed(1)} t/s en baseline es incómodo. Probá cuantización más agresiva o un modelo más pequeño.`});
  else if(BASELINE>30) os.push({t:'good',g:'⚡',ti:'TPS excelente',tx:`${BASELINE.toFixed(1)} t/s — setup muy bien configurado. Podés aumentar <code>--ctx-size</code> para aprovechar más VRAM.`});
  else if(BASELINE>15) os.push({t:'good',g:'✓',ti:'TPS sólido',tx:`${BASELINE.toFixed(1)} t/s es cómodo para conversación. Para más, probá reducir la cuantización del KV cache o activar flash attention.`});
  if(AVG_UTIL>0&&AVG_UTIL<70) os.push({t:'warn',g:'📊',ti:'GPU subutilizada — cuello de botella externo',tx:`Utilización GPU al ${AVG_UTIL}%. El límite está en <strong>CPU o disco</strong>, no en la GPU. Verificá que el modelo esté completo en VRAM (<code>-ngl 99</code>).`});
  if(GPU_VRAM>=24) os.push({t:'good',g:'🏆',ti:'24GB VRAM — setup privilegiado',tx:`Con 24GB podés correr <strong>13B en Q8</strong> o <strong>34B en Q4_K_M</strong> con contextos de hasta 64K. Probá <code>--ctx-size 32768 --flash-attn on</code>.`});
  os.push({t:'info',g:'🔮',ti:'Próximo paso — entrenar el Oráculo',tx:`Con más runs en el chronicle, ejecutá <code>python typhon.py train</code> para que el modelo XGBoost prediga performance y VRAM para tu hardware.`});
  const c=document.getElementById('omens-container');
  os.forEach(o=>{const d=document.createElement('div');d.className=`omen omen-${o.t}`;d.innerHTML=`<div class="omen-glyph">${o.g}</div><div><div class="omen-title">${o.ti}</div><div class="omen-text">${o.tx}</div></div>`;c.appendChild(d);});
})();
...
def generate_html(d: dict) -> str:
    ...
    power_str  = f" Consumo pico: {d['peak_power']:.0f} W." if d["peak_power"] else ""
    ...
    replacements = {
        ...
        "%%VRAM_TOTAL_MB%%":  str(d["vram_total_mb"]),
        "%%VRAM_PCT%%":       str(vram_pct),
        "%%GPU_VRAM_GB%%":    str(d["gpu_vram_gb"]),
